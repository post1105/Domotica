...
namespace Domotica
{
    ...
    public class MainActivity : Activity
    {
        ...
        public TextView textViewChangePinStateValue, textViewChangePin2StateValue, textViewChangePin3StateValue, textViewSensorValue, textViewSensor2Value, textViewDebugValue;
        EditText editTextIPAddress, editTextIPPort, editTextLichtTres, editTextDrukTres, editTextTime;

        Timer timerClock, timerSockets, timerTimer, timerSensors /*timerSensors*/;             // Timers   
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            textViewChangePin2StateValue = FindViewById<TextView>(Resource.Id.textViewChangePin2StateValue);
            textViewChangePin3StateValue = FindViewById<TextView>(Resource.Id.textViewChangePin3StateValue);
            textViewSensorValue = FindViewById<TextView>(Resource.Id.textViewSensorValue);
            textViewSensor2Value = FindViewById<TextView>(Resource.Id.textViewSensor2);
            ...
            editTextTime = FindViewById<EditText>(Resource.Id.editTime);
            editTextTime.KeyPress += (object sender, View.KeyEventArgs e) => {
                e.Handled = false;
                if (e.Event.Action == KeyEventActions.Down && e.KeyCode == Keycode.Enter) {
                    Toast.MakeText (this, editTextTime.Text, ToastLength.Short).Show ();
                    e.Handled = true;
            }
};
            ...
            commandList.Add(new Tuple<string, TextView>("r", textViewChangePin2StateValue));
            commandList.Add(new Tuple<string, TextView>("q", textViewChangePin3StateValue));
            ...
            /*timerTimer = new System.Timers.Timer() { Interval = 1000, Enabled = true };   //Ruben
            timerTimer.Elapsed += (obj, args) =>
            {
                if (editTextTime.ToString() == textViewTimerStateValue.Text)
                {
                    socket.Send(Encoding.ASCII.GetBytes("v"));
                }
            };*/

            /*timerSensors = new System.Timers.Timer() { Interval = 1000, Enabled = true };   //Ruben
            timerSensors.Elapsed += (obj, args) =>
            {
                string a = editTextLichtTres.ToString();
                string b = textViewSensorValue.ToString();
                int c = Convert.ToInt16(a);
                int d = Convert.ToInt16(b);
                string e = editTextDrukTres.ToString();
                string f = textViewSensor2Value.ToString();
                int g = Convert.ToInt16(e);
                int h = Convert.ToInt16(f);
                if (d >= c && textViewChangePinStateValue.Text == "OFF")
                {
                    socket.Send(Encoding.ASCII.GetBytes("t"));
                }
                if (d < c && textViewChangePinStateValue.Text == " ON")
                {
                    socket.Send(Encoding.ASCII.GetBytes("t"));
                }
                if (h >= g && textViewChangePin2StateValue.Text == "OFF")
                {
                    socket.Send(Encoding.ASCII.GetBytes("u"));
                }
                if (h < g && textViewChangePin2StateValue.Text == " ON")
                {
                    socket.Send(Encoding.ASCII.GetBytes("u"));
                }
            };*/
            ...
            if (buttonChangePin2State != null)
            {
                buttonChangePin2State.Click += (sender, e) =>
                {
                    if (connector == null) // -> simple sockets
                    {
            ...
            if (buttonChangePin3State != null)
            {
                buttonChangePin3State.Click += (sender, e) =>
                {
                    if (connector == null) // -> simple sockets
                    {
        ...
        public void UpdateConnectionState(int state, string text)
        {
            ...
            RunOnUiThread(() =>
            {
                ...
                buttonChangePin2State.Enabled = butPinEnabled;
            });
            RunOnUiThread(() =>
            {
                textViewServerConnect.Text = text;
                ...
                buttonChangePin3State.Enabled = butPinEnabled;
            });
        }
        ...
        public void UpdateGUI(string result, TextView textview)
        {
            RunOnUiThread(() =>
            {
                if (result == "OFF")
                {
                    textViewChangePinStateValue.SetTextColor(Color.Red);
                    textViewChangePinStateValue.Text = "OFF";
                }
                else if (result == " ON")
                {
                    textViewChangePinStateValue.SetTextColor(Color.Green);
                    textViewChangePinStateValue.Text = " ON";
                }
                else if (result == "UIT")
                {
                    textViewChangePin2StateValue.SetTextColor(Color.Red);
                    textViewChangePin2StateValue.Text = "OFF";
                }
                else if (result == "AAN")
                {
                    textViewChangePin2StateValue.SetTextColor(Color.Green);
                    textViewChangePin2StateValue.Text = " ON";
                }
                else if (result == "NEE")
                {
                    textViewChangePin3StateValue.SetTextColor(Color.Red);
                    textViewChangePin3StateValue.Text = "OFF";
                }
                else if (result == " JA")
                {
                    textViewChangePin3StateValue.SetTextColor(Color.Green);
                    textViewChangePin3StateValue.Text = " ON";
                }
                else
                {
                    textViewSensorValue.Text = result;
                }
            });
        }
