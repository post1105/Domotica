...
namespace Domotica
{
    ...
    public class MainActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            {
                if (textViewSensorValue.ToInt16 >= editTextDrukTres.ToInt16)
                {
                    socket.Send(Encoding.ASCII.GetBytes("u"));
                }
                if (textViewSensor2Value.ToInt16 >= editTextLichtTres.ToInt16)
                {
        ...
        public void UpdateGUI(string result, TextView textview)
        {
            RunOnUiThread(() =>
            {
                ...
                if (result == "UIT") { textview.Text = "OFF"; }
                if (result == "AAN") { textview.Text = " ON"; }
        ...
        public void UpdateGUI(string result, TextView textview)
        {
            RunOnUiThread(() =>
            {
                if (result == "NEE") textview.SetTextColor(Color.Red);
                else if (result == " JA") textview.SetTextColor(Color.Green);
                else textview.SetTextColor(Color.White);
                if (result == "NEE") { textview.Text = "OFF"; }
                if (result == " JA") { textview.Text = " ON"; }
            });
        }
