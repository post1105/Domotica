            ...
            try {
				...
				while ((Thread.VolatileRead (ref _threadStarted) == 1) && socket.Connected) {
                    ...
                    if (bytesRead == 4) { 
                        ...
                        {
                            mainActivity.UpdateGUI(result, mainActivity.textViewChangePin2StateValue);
                        }
                        if (result == "NEE" || result == " JA")
                        {
                            mainActivity.UpdateGUI(result, mainActivity.textViewChangePin3StateValue);
                        }
					}
					Thread.Sleep (10);
				}
